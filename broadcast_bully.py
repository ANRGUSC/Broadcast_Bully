"""
    Broadcast Bully:
        ...
        -sends and receives messages containing IP addresses, victory messages,
            election messages, and timerout messages
...
async def fail_detect(state):
    ...
    Accepts: state which is an empty object from the udp_broadcast class
    Returns: nothing, it pops nodes from dictionary if node fails and
             notifies if popped node is coordinator, coordinator failed
    """
    failed_key = None
    while True:
...
async def beacon_broadcast(state):
    ...
    # create a datagram endpoint with broadcast
...
async def timeout(period, state):
    ...
    Accepts: period object from console input,
             and state which is an empty object from the udp_broadcast class
    Returns: nothing, it sets broadcast to false if called after timeout,
...
async def main():
    """
    Async main method to initialize objects, get console input,
         and gather asycn methods to call and run asynchronously
    """
    state = type('', (), {})()
    state.nodes = {}
